...
3. **AI Note Conversion**: Automatically transforms raw AI-generated notes from Quark or Baidu into clickable Sync Vault links.
...
### 2. External AI Notes (Quark/Baidu)
Sync Vault can convert notes generated by cloud disk AI assistants into native seek links.
- **Quark Format**: Supports Markdown anchors `[12:41](#?seek_t=761)` or trailing timestamps `Description 12:41`.
- **Baidu Format**: Supports leading timestamps `00:08 Description`.
...
3. **External Note Conversion**:
    - When a user pastes raw AI notes, identify the source (Quark/Baidu).
    - Parse timestamps (HH:MM:SS or MM:SS) and calculate total seconds.
    - Wrap the timestamp in the `obsidian://cloud-video-seek` protocol.
